...
async function predictDisease(req, res) {
  ...
  try {
    const url = 'https://www.ailabapi.com/api/portrait/analysis/skin-disease-detection';
    console.log('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>');
    console.log(req.file);
    const base64Image = req.file ? req.file.buffer.toString('base64') : undefined;
    ...
    const resp = await axios.post(url, form, {
      headers: { ...form.getHeaders(), 'ailabapi-api-key': process.env.AILAB_API_KEY },
    });
    ...
    const saved = await models.SkinAnalysis.create({

...
async function getSTTResultFromBuffer(audioFile) {
  ...
  const config = {
    ...
    speechContexts: [
      {
        phrases: ["자동차", "모자", "나무", "시계"], // 모델이 강조할 단어들
        boost: 100.0, // 가중치 (1.0~20.0 사이 권장)
      },
    ],
    model: "latest_short", // Chirp 모델
  };
...
function calculateScore(answers, correct, options = {}) {
  //const { nGramSize = 2, similarityThreshold = 0.5, useNGram = true } = options;
  const { similarityThreshold = 0.5 } = options;
  let score = 0;
  const sttText = answers.join("").replace(/\s+/g, "");

  console.log("STT 단어 배열:", answers);
  correct.forEach((word) => {
    const similarity = stringSimilarity.compareTwoStrings(sttText, word);
    console.log(
      `정답 단어: "${word}", STT 전체: "${sttText}", similarity: ${similarity.toFixed(3)}`
    );
    if (similarity >= similarityThreshold) {
      score += 1;
      console.log(`→ "${word}" 인식됨!`);
    }
  });

  console.log("계산된 점수:", score);
  return score;
}
...
async function gradeSTTHandler(
  ...
  try {
    ...
    console.log("STT 전체 텍스트:", sttResult);
    //const answers = sttResult.split(/\s+/);
    const answers = [sttResult];
    let score = calculateScore(answers, correct, options);
    if (message == "이름 대기") {
      score *= 2;
    }
...
function gradeRegistration(req, res) {
  gradeSTTHandler(
    ...
    res,
    ["나무", "자동차", "모자"],
    { nGramSize: 2, similarityThreshold: 0.5 },
    "기억 등록"
  );
}

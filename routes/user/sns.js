...
async function handleCallback(req, res, provider) {
    ...
    try {
        ...
        if (process.env.NODE_ENV === 'development') { // 테스트
            ...
            const deepLink = `yoyangi://${provider}login?token=${encodeURIComponent(token)}&refreshToken=${encodeURIComponent(snsInstance.refresh_token)}`;
